...
def create_clean_manuscript():
    ...
    # PART 1: Title page through Chapter 5 (from complete doc)
    # Find where Chapter 5 content ends
    ch5_start = 0
    ...
    # Copy everything up to and including Chapter 5 content
    for i in range(ch5_start):
        text = complete_doc.paragraphs[i].text.strip()
        if text:
            new_doc.add_paragraph(text)
    
    # Add Conceptual Framework image after the Conceptual Framework section
    ...
    for i in range(ch5_start, len(complete_doc.paragraphs)):
        ...
        if text:
            ...
            # Add image after "OUTPUT STAGE" or after "comprehensive"
            if 'comprehensive' in text.lower() and i < ch5_start + 50:
    ...
    # PART 2: Chapter 6 - Methods and Procedures
    # Skip the "CHAPTER 6" and header paragraphs, get the content
    ch6_content = []
    for p in chapter6_doc.paragraphs:
        text = p.text.strip()
        if text and text != 'CHAPTER 6' and 'METHODS AND PROCEDURES' not in text.upper():
            ch6_content.append(text)
    
    # Add Chapter 6 header
    new_doc.add_paragraph('CHAPTER 6')
    new_doc.add_paragraph('METHODS AND PROCEDURES')
    new_doc.add_paragraph('')
    
    # Add content, inserting image at Design Concept
    for text in ch6_content:
        new_doc.add_paragraph(text)
    ...
    # PART 3: Chapter 7 - Results and Discussion
    ch7_content = []
    for p in chapter7_doc.paragraphs:
        text = p.text.strip()
        if text and text != 'CHAPTER 7' and 'RESULTS AND DISCUSSION' not in text.upper() and 'CHAPTER 8' not in text.upper() and 'CONCLUSIONS' not in text.upper():
            ch7_content.append(text)
    
    new_doc.add_paragraph('CHAPTER 7')
    new_doc.add_paragraph('RESULTS AND DISCUSSION')
    new_doc.add_paragraph('')
    
    for text in ch7_content:
        new_doc.add_paragraph(text)
    
    # Page break before Chapter 8
    new_doc.add_page_break()
    ...
    print(f'Final complete manuscript saved to: {output_path}')
